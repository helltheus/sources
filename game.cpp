...
bool Game::combatChangeHealth(const CombatParams& params, Creature* attacker, Creature* target, int32_t healthChange, bool force)
{
	...
	if(healthChange > 0)
	{
		...
		// START BONUS STORAGE HEALING LIFE
        std::string value;
        target->getStorage("170301", value); // SET STORAGE HERE
        int32_t storageBonus = atoi(value.c_str());
      
        if (storageBonus >= 1)
            healthChange = healthChange * (1 + (storageBonus * 0.1)); // 0.1 = 10%
      
        // END BONUS STORAGE HEALING LIFE
		bool deny = false;
	...
	CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	...
	if(manaChange > 0)
	{
		// START BONUS STORAGE HEALING MANA
        std::string value;
        target->getStorage("170301", value); // SET STORAGE HERE
        int32_t storageBonus1 = atoi(value.c_str());
      
        if (storageBonus1 >= 1)
            manaChange = manaChange * (1 + (storageBonus1 * 0.1)); // 0.1 = 10%
      
        // END BONUS STORAGE HEALING Mana
